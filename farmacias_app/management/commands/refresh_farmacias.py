import json
import requests
from farmacias_app.models import Farmacia, Comuna
...
class Command(BaseCommand):
    help = 'Refreshes data of farmacias_app_farmacia'
    ...
    def populate_farmacias_turno(self):
        for element in self.data:
            ...
            f.apertura = datetime.strptime(f"{element['fecha']} {element['funcionamiento_hora_apertura']}", '%Y-%m-%d '
                                                                                                            '%H:%M:%S')

            f.cierre = datetime.strptime(f"{element['fecha']} {element['funcionamiento_hora_cierre']}",
